...
function GammaPanel({ stocks }) {
  ...
  const fetchGamma = useCallback(async (sym) => {
    setLoading(true);
    setError("");
    setData(null);
    try {
  ...
  useEffect(() => { fetchGamma(symbol); }, [symbol, fetchGamma]);
  ...
  const signal = (() => {
    ...
    const regimeColor = netGex >= 0 ? "#10b981" : "#ef4444";
    let msg = netGex >= 0
      ? `Market makers are long gamma — they sell rallies and buy dips. Price tends to gravitate toward the $${gammaWall} gamma wall.`
      : `Market makers are short gamma — they chase moves. Expect amplified swings. Watch the $${flipLevel ?? putWall} flip level.`;
    return { regime, regimeColor, msg, flipLevel, gammaWall, putWall, callWall };
  })();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <select value={symbol} onChange={e => setSymbol(e.target.value)} style={{ ...inputStyle, flex: 1 }}>
          {TICKERS.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        <button onClick={() => fetchGamma(symbol)} style={{
          background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)",
          borderRadius: 6, padding: "7px 12px", color: "#9ca3af", fontSize: 11,
          cursor: "pointer", fontWeight: 600,
        }}>↻</button>
      ...
      {error && (
        <div style={{ color: "#ef4444", fontSize: 11, padding: "10px 12px", borderRadius: 8, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)" }}>
          {error}
        </div>
      )}
      ...
      {data && signal && (
        <>
          ...
          <div style={{ padding: "12px 14px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: `1px solid ${signal.regimeColor}33` }}>
            ...
            <div style={{ height: 4, background: "rgba(255,255,255,0.07)", borderRadius: 2, marginBottom: 10 }}>
              <div style={{
                ...
                background: signal.regimeColor,
                width: `${Math.min(Math.abs(data.netGex) / (maxAbsGex * gexList.length / 8) * 100, 100)}%`,
          ...
          <div style={{ padding: "10px 14px", borderRadius: 8, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ fontSize: 10, color: "#4b5563", letterSpacing: "0.1em", marginBottom: 8 }}>KEY LEVELS</div>
            {[
              { label: "Gamma Wall", val: signal.gammaWall, color: "#10b981", desc: "Strongest magnet" },
              { label: "Flip Level", val: signal.flipLevel, color: "#f59e0b", desc: "Regime changes here" },
              { label: "Call Wall", val: signal.callWall, color: "#a5b4fc", desc: "Resistance" },
              { label: "Put Wall", val: signal.putWall, color: "#f87171", desc: "Support" },
              { label: "Current", val: spot ? `$${spot.toFixed(2)}` : null, color: "#f9fafb", desc: "Price now", raw: true },
            ].filter(x => x.val).map(({ label, val, color, desc, raw }) => (
              <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                <div>
                  <span style={{ fontSize: 11, color, fontWeight: 700 }}>{label}</span>
                  <span style={{ fontSize: 10, color: "#374151", marginLeft: 6 }}>{desc}</span>
                ...
                <span style={{ fontSize: 12, fontFamily: "monospace", color, fontWeight: 700 }}>
                  {raw ? val : `$${parseFloat(val).toFixed(0)}`}
                </span>
              </div>
            ))}
          </div>
          ...
          {/* GEX Bar Chart */}
          <div style={{ padding: "10px 14px", borderRadius: 8, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ fontSize: 10, color: "#4b5563", letterSpacing: "0.1em", marginBottom: 8 }}>GEX BY STRIKE</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 2, maxHeight: 280, overflowY: "auto" }}>
              {[...gexList].reverse().map(row => {
                const isSpot = spot && Math.abs(row.strike - spot) < 2.5;
                const barPct = Math.max(1, (Math.abs(row.gex) / maxAbsGex) * 100);
                const color = row.gex >= 0 ? "#10b981" : "#ef4444";
                return (
                  <div key={row.strike} style={{ display: "flex", alignItems: "center", gap: 6,
                    background: isSpot ? "rgba(245,158,11,0.08)" : "none",
                    borderLeft: isSpot ? "2px solid #f59e0b" : "2px solid transparent",
                    paddingLeft: 4,
                  }}>
                    <span style={{ fontSize: 10, fontFamily: "monospace", color: isSpot ? "#f59e0b" : "#4b5563", width: 42, flexShrink: 0 }}>
                      {row.strike % 1 === 0 ? row.strike.toFixed(0) : row.strike.toFixed(1)}
                    </span>
                    <div style={{ flex: 1, height: 10, background: "rgba(255,255,255,0.04)", borderRadius: 2, position: "relative" }}>
                      <div style={{
                        position: "absolute", top: 0, bottom: 0, left: 0,
                        width: `${barPct}%`, borderRadius: 2,
                        background: `${color}${row.gex >= 0 ? 'cc' : '99'}`,
                      }} />
                    </div>
                    <span style={{ fontSize: 9, fontFamily: "monospace", color, width: 38, flexShrink: 0, textAlign: "right" }}>
                      {fmtGex(row.gex)}
                    </span>
                  </div>
                );
